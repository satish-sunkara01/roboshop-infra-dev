resource "aws_cloudfront_distribution" "saachi" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.saachi.domain_name
        zone_id = aws_cloudfront_distribution.saachi.hosted_zone_id
      }
    }
  ]
}